...
FSimpleConnetion::FSimpleConnetion()
	:State(ESimpleConnetionLinkType::LINK_Free)
	, Socket(NULL)
	, bLock(false)
	, HeartTime(0.0)
	, LastTime(0.0)
	...
	, Manage(nullptr)
{
	rCode = SimpleEncryptionAndDecryption::m_rCode;
...
void FSimpleConnetion::Tick(float DeltaSeconds)
{
	...
	{
		if(State == ESimpleConnetionLinkType::LINK_ConnectSecure)
		{
			if(bHeartBeat)
			{
...
void FSimpleConnetion::SendHeartBeat()
{
	if(FSimpleChannel* Channel = GetMainChannel())
	{
		...
		{
			int32 time = 0;
			SIMPLE_PROTOCOLS_SEND(SP_Heart, time)
...
{
	SimpleEncryptionAndDecryption::Encryption(InData,rCode);
}
...
FSocket* FSimpleConnetion::GetSocket()
{
	return Socket;
}
