...
AMMOARPGMonster* UMMOARPGGameInstance::SpawnMonsterDebug(int32 MonsterId)
{
    ...
    if (!BPClass)
    {
        UE_LOG(LogTemp, Error, TEXT("MonsterBlueprint is NULL for id=%d (确保 DataTable 填的是 *_C 类路径)"), MonsterId);
        return nullptr;
	}
	...
	Params.bNoFail = false; // 可设 true 看看是否有 ensure
    ...
    if (USkeletalMeshComponent* Skel = M->GetMesh())
    {
        ...
        // AnimClass（无论你是 TSoftClassPtr<UAnimInstance> 还是 UAnimBlueprint）
