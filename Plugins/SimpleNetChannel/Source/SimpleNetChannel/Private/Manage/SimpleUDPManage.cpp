...
bool FSimpleUDPManage::Init(uint32 InIP, uint32 InPort)
{
	...
	if(Net.LocalConnetion)
	{
		...
		Socket = SocketSubsystem->CreateSocket(NAME_DGram, TEXT("Flower"));
		if(!Socket)
		{
		...
		else if(LinkState == ESimpleNetLinkState::LINKSTATE_LISTEN)
		{
			...
			Net.LocalConnetion->SetState(ESimpleConnetionLinkType::LINK_ConnectSecure);
...
void FSimpleUDPManage::Listen()
{
	...
	if(bRecvFrom)
	{
		...
		if(LinkState == ESimpleNetLinkState::LINKSTATE_LISTEN)
		{
			...
			{
				SimpleEncryptionAndDecryption::Decryption(Data, BytesRead, NewConnetion->rCode);
				uint8* NewRecvData = nullptr;
				int32 NewRecvLen = 0;
				if(IsCompletePackage(BytesRead, Data, NewConnetion, InGUID, NewRecvData, NewRecvLen))
				{
					if(NewConnetion->GetState() == ESimpleConnetionLinkType::LINK_ConnectSecure)
					{
						NewConnetion->Analysis(NewRecvData, NewRecvLen);//Analysis
					}
					else
					{
						VerificationConnectionInfo(NewConnetion, NewRecvData, NewRecvLen);
					}
				}
			}
			else
			{
				...
				{
					SimpleEncryptionAndDecryption::Decryption(Data, BytesRead, TmpConnetion->rCode);
					uint8* NewRecvData = nullptr;
					int32 NewRecvLen = 0;
					if(IsCompletePackage(BytesRead, Data, TmpConnetion, InGUID, NewRecvData, NewRecvLen))
					{
						VerificationConnectionInfo(TmpConnetion, NewRecvData, NewRecvLen);
		...
		else if(LinkState == ESimpleNetLinkState::LINKSTATE_CONNET) //The client can parse the data directly
		{
			...
			if(IsCompletePackage(BytesRead, Data, Net.LocalConnetion, InGUID, NewRecvData, NewRecvLen))
			{
				...
				{
					VerificationConnectionInfo(Net.LocalConnetion, NewRecvData, NewRecvLen);
				}
			}
		}
...
bool FSimpleUDPManage::IsCompletePackage(int32 InRecvNum,
										 uint8* InData,
