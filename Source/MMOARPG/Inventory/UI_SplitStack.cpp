...
uint32 UUI_SplitStack::ResolveBuyPropId() const
{
	...
	if (const uint32* Found = FallbackVendorIdMap.Find(NormalizedIdText))
	{
		return *Found;
	}

	UE_LOG(MMOARPG, Warning, TEXT("[VendorUI] ResolveBuyPropId failed [RawID:%s TrimID:%s NormID:%s Index:%d]"),
		*RawIdText,
		*IdText,
		*NormalizedIdText,
		m_ItemData->Index);
