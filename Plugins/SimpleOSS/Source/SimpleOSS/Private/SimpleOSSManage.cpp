...
FString FSimpleOSSManage::FOSS::GetSymlink(const FString &InBucketName, const FString &InLinkName)
{
	...
	return UTF8_TO_TCHAR(target_object_name);
}

bool FSimpleOSSManage::FOSS::GeneratePresignedUrl(FString &URL, const FString &InBucketName, const FString &InObjectName)
{
	CREATE_CONFIGURATION_AND_STORAGE;
	...
	int one_hour = 3000;
	apr_time_t now = apr_time_now(); /* 单位：微秒 */
	int64_t expire_time = now / 1000000 + one_hour;
...
bool FSimpleOSSManage::FOSS::GetObjectToMemory(
	...
	OnceStringCallBack StartCallback /*= nullptr*/, 
	OnceStringCallBack EndCallback /*= nullptr*/)
{
	...
	{
		FString URL;
		if (GeneratePresignedUrl(URL, InBucketName, InObjectName))
		{
			StartCallback(URL);
		}
	...
	auto DownLoadString =[&]()
	{
		...
		if (InProgressCallback)
		{
			EndCallback(FString(UTF8_TO_TCHAR(buf)));
		}
		else
		{
			ContextString = UTF8_TO_TCHAR(buf);
		}
	};
...
bool FSimpleOSSManage::FOSS::GetObjectToMemoryByURL(
	const FString& URL,
	FString &ContextString,
	FRange Range /*= FRange()*/,
	ProgressCallback InProgressCallback /*= nullptr*/,
	...
	OnceStringCallBack EndCallback /*= nullptr*/)
{
	...
	STRING_TO_AOS_STRING_T(InUrl, URL);
	...
	if (InProgressCallback)
	{
		...
		resp_status = oss_get_object_to_buffer_by_url(oss_client_options, &InUrl, headers, params, &buffer, &resp_headers, InProgressCallback);
	}
	else
	{
		resp_status = oss_get_object_to_buffer_by_url(oss_client_options, &InUrl, headers, params, &buffer, &resp_headers, NULL);
	}
	...
	/*Determine whether the upload is successful. */
	CHECK_OSS_RESULT(DownLoadString();
		OSS_SUCCESS_MSG("get object to buffer by url succeeded\n");,
		OSS_ERROR_MSG("get object to buffer by url failed\n"););
	...
	ProgressCallback InProgressCallback /*= nullptr*/)
{
	...
	/*Download the file. If the specified local file exists, it will be overwritten. If it does not exist, it will be created. */
	if (InProgressCallback)
	{
...
bool FSimpleOSSManage::FOSS::GetObjectToLocalByURL(
	const FString& URL,
	const FString& InLocalPaths,
	FRange Range /*= FRange()*/,
	ProgressCallback InProgressCallback /*= nullptr*/,
	OnceStringCallBack InStartCallback , 
	OnceStringCallBack InEndCallback)
{
	FScopeLock ScopeLock(&Mutex);
	...
	if (!NewInlocalPaths.Contains("."))
	{
		FString FileName = FPaths::GetCleanFilename(URL);
		if (InLocalPaths[InLocalPaths.Len() - 1] == '/')
		{
			NewInlocalPaths += FileName;
		}
		else
	...
	{
		InStartCallback(URL);
		resp_status = oss_get_object_to_file_by_url(oss_client_options, &InUrl, headers, params, &file, &resp_headers, InProgressCallback);
		InEndCallback(URL);
	}
	else
	{
		resp_status = oss_get_object_to_file_by_url(oss_client_options, &InUrl, headers, params, &file, &resp_headers,NULL);
	}	
...
void FSimpleOSSManage::FOSS::AsyncPutObjectByURL(
	const FString& URL, 
	const FString& InLocalPaths,
...
void FSimpleOSSManage::FOSS::AsyncPutObjectByMemory(const FString &InBucketName, const FString &InObjectName /*= FString()*/, const FString &Data,  ProgressCallback InProgressCallback /*= nullptr*/, const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/)
{
	auto Lamada = [=]()
	{
		PutObjectByMemory(
...
void FSimpleOSSManage::FOSS::AsyncPutObjectMemoryByURL(const FString& URL, const FString &Data, const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/, ProgressCallback InProgressCallback /*= nullptr*/)
{
	auto Lamada = [=]()
	{
		PutObjectMemoryByURL(
			URL,
			Data,
			OSSMeta,
			InProgressCallback);
...
bool FSimpleOSSManage::FOSS::ListObjects_Internal(const FString &InBucketName, const FString &Delimiter, const FString &Prefix, TArray<FOSSObjectInfo> &ObjectName, int32 MaxNumber /*= 100*/)
{
	...
	/*Enumerates the files that contain the specified prefix. */
	ANSICHAR *nextMarker = "";

	/*Set the forward slash (/) as the separator for the folder. */
	...
	const TMap<FString, FString> &OSSMeta)
{
	...
	STATEMENT_VISITSUCCESS
	CHECK_OSS_RESULT_NOBOOL(
	InUploadId = UTF8_TO_TCHAR(upload_id.data);
	auto Lamada = [=]()
	{
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToLocalByURL(
	const FString& URL, 
	const FString& InLocalPaths,
	FRange Range /*= FRange()*/,
	ProgressCallback InProgressCallback /*= nullptr*/,
	OnceStringCallBack InStartCallback /*= NULL*/,
	OnceStringCallBack InEndCallback /*= NULL*/)
{
	auto Lamada = [=]()
	{
		GetObjectToLocalByURL(
			URL,
			InLocalPaths,
			Range,
			InProgressCallback, 
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToMemory(const FString &InBucketName, const FString &InObjectName, FRange Range /*= FRange()*/, ProgressCallback InProgressCallback /*= nullptr*/, OnceStringCallBack InStartCallback /*= NULL*/, OnceStringCallBack InEndCallback /*= NULL*/)
{
	auto Lamada = [=]()
	{
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToMemoryByURL(const FString& URL, FRange Range /*= FRange()*/, ProgressCallback InProgressCallback /*= nullptr*/, OnceStringCallBack InStartCallback /*= NULL*/, OnceStringCallBack InEndCallback /*= NULL*/)
{
	auto Lamada = [=]()
	{
		FString ContextString;
		GetObjectToMemoryByURL(
			URL,
			ContextString,
			Range,
			InProgressCallback, 
...
bool FSimpleOSSManage::FOSS::PutObjectByURL(const FString& URL, const FString& InLocalPaths, const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/, ProgressCallback InProgressCallback /*= nullptr*/)
{
	FScopeLock ScopeLock(&Mutex);
	...
	if (!NewInlocalPaths.Contains("."))
	{
		FString FileName = FPaths::GetCleanFilename(NewInlocalPaths);
		if (InLocalPaths[InLocalPaths.Len()] == '/')
		{
			NewInlocalPaths += FileName;
		}
		else
	...
	aos_table_t *headers = NULL;
	aos_list_t resp_body;
	resp_body.next = nullptr;
	resp_body.prev = nullptr;
	...
	if (InProgressCallback)
	{
		resp_status = oss_put_object_from_file_by_url(oss_client_options, &InUrl, &file, headers, &resp_headers, InProgressCallback);
	}
	else
	{
		resp_status = oss_put_object_from_file_by_url(oss_client_options, &InUrl, &file, headers, &resp_headers,NULL);
	}

	CHECK_OSS_RESULT(
		OSS_SUCCESS_MSG("Upload from local %s succeeded .", *URL),
		OSS_ERROR_MSG("Upload from local %s fail .", *URL));
...
bool FSimpleOSSManage::FOSS::PutObjectMemoryByURL(
	const FString& URL,
	const FString &Data,
	const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/,
	ProgressCallback InProgressCallback /*= nullptr*/)
{	
	...
	if (InProgressCallback)
	{
		resp_status = oss_put_object_from_buffer_by_url(oss_client_options,&InUrl,&buffer, headers, &resp_headers, InProgressCallback);
	}
	else
	{
		resp_status = oss_put_object_from_buffer_by_url(oss_client_options, &InUrl, &buffer, headers, &resp_headers,NULL);
	}

	/*Determine whether the upload is successful. */
	CHECK_OSS_RESULT(
		OSS_SUCCESS_MSG("put object from buffer by url succeeded\n."),
		OSS_ERROR_MSG("put object from buffer by url failed\n"))
